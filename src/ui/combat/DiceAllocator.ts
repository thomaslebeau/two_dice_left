...
export class DiceAllocator {
  ...
  private _allocs = new Map<number, number>();
  ...
  getAllocations(): Allocation[] {
    return [...this._allocs].map(([di, ei]) => ({ equipmentIndex: ei, dieValue: this._dice[di].value }));
  }
  ...
  handlePointerUp(): void {
    ...
    for (const s of this._slots) {
      ...
      if (s.isCompatible(die.value)) { this._place(die.dieIndex, s.equipmentIndex); placed = true; }
      else die.shake();
      break;
    }
  ...
  handleSlotTap(si: number): void {
    if (!this._enabled) return;
    const s = this._slots[si];
    if (!s || s.slotState === 'filled') return;
    for (const d of this._dice) {
      if (this._allocs.has(d.dieIndex)) continue;
      if (!s.isCompatible(d.value)) continue;
      this._place(d.dieIndex, si);
      this._highlight();
      this.onChange?.();
      return;
    }
  ...
  private _place(di: number, si: number): void {
    const die = this._dice[di], slot = this._slots[si];
    if (!die || !slot) return;
    this._allocs.set(di, si);
  ...
  private _undo(di: number): void {
    const si = this._allocs.get(di);
    if (si === undefined) return;
    this._allocs.delete(di);
    this._slots[si]?.removeDie();
